...
from .models import ServicesOffered, ClientProfile, ClientAppointments, Invoices
...
def create_customer_appointment(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if ClientAppointments.objects.filter(appointment_date=appointment.appointment_date, time_of_day=appointment.time_of_day).exists():
                messages.error(request, 'Date has already been taken, Pick another  Day')
                return redirect('customer_appointment')
            appointment.save()
            
            print(appointment.user, appointment.service)
